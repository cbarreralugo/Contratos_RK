    ...
    public class Email
    {
        public void SendEmail(string to, string subject, string body, bool attachFile = false)
        {
            try
            {
                ...
                {
                    Port = 587,
                    Credentials = new NetworkCredential("sam_contratos_prueba@outlook.com", "K10sm4rt"),
                    EnableSsl = true,
                };
                ...
                {
                    From = new MailAddress("sam_contratos_prueba@outlook.com"),
                    Subject = subject,
                    IsBodyHtml = true
                };
                ...
                string htmlBody = GetHtmlBody(body);

                // Crear la vista alternativa HTML
                AlternateView avHtml = AlternateView.CreateAlternateViewFromString(htmlBody, null, MediaTypeNames.Text.Html);

                // Adjuntar la imagen
                string imagePath = AppDomain.CurrentDomain.BaseDirectory + "data/image-1.png";
...
<p style=""font-size: 14px; line-height: 160%;"">Procesado por username</p>
  </div>
...
<p style=""font-size: 14px; line-height: 150%;"">fecha</p>
  </div>
...
<!--[if (mso)|(IE)]></td><![endif]-->
  ...
  <!--[if IE]></div><![endif]-->
                ...
                return htmlTemplate.Replace("{nombre_user}", "Nombre del Usuario").Replace("{body}", body);
            }
            catch (FormatException ex)
            {
